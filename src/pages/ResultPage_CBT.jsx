        ...
        {!hasResult ? (
          ...
          <>
            <header style={styles.header}>
              <div>
                <p style={styles.kicker}>{result.isTestMode ? "測驗結果" : "練習結果"}</p>
                <h1 className="cbt-title" style={styles.title}>
            ...
            <section style={styles.overviewCard}>
              ...
              <div style={styles.overviewCopy}>
                ...
                <p style={styles.taskText}>{result.mainSummary}</p>
                <p style={styles.starNotice}>
                  星星代表本次完成表現。請搭配下方觀察重點一起看，不代表單一次診斷。
                </p>
              </div>
            </section>
...
function buildParentFriendlyResult(historyInput, state = {}, mode = "training") {
  ...
  const maxAttemptSpan = Math.max(
    independentSpan,
    assistedSpan,
    ...history.map(getTrialLength),
    0
  );

  const stopReasonText = getStopReasonText(safeState.stopReason);
  const level = getTrainingLevel({ maxAttemptSpan, independentSpan, difficulty: safeState.difficulty });
  ...
  const overviewTitle = getOverviewTitle({ stars, weightedAccuracyPercent, totalTrials, isTestMode });
  ...
  const systemSummary = getSystemSummary(safeState, scoring);
  const insights = buildInsightCards({ isTestMode, pattern, independentSpan, assistedSpan, weightedAccuracyPercent });
  const nextSteps = [
    ...buildNextSteps({
      suggestedSpan,
      independentSpan,
  ...
  return {
    ...
    cards: isTestMode ? [
      ...
      },
      {
        ...
        icon: "🏁",
        title: "結束原因",
        value: getStopReasonLabel(safeState.stopReason),
        note: stopReasonText,
      },
      {
        key: "completion",
      ...
      { key: "level", icon: "🪜", title: "目前訓練層級", value: level.title, note: level.description, highlight: true },
      { key: "stars", icon: "⭐", title: "本次星等解讀", value: `${stars} 星`, note: getTrainingStarDescription(stars) },
      { key: "pattern", icon: "🔎", title: "主要表現型", value: pattern.title, note: pattern.description },
      {
        key: "support",
        icon: "🤝",
        title: "提示依賴程度",
        value: assistedSpan > independentSpan || replayRate >= 0.35 ? "需要留意" : "相對穩定",
        note: assistedSpan > independentSpan
          ? "提示後能完成更長路線，能力正在形成，但目前仍需要外在線索協助。"
          : "這次提示前後的完成步數接近，可繼續觀察是否能穩定維持。",
      },
      { key: "independent", icon: "👣", title: "自己記住", value: independentSpan > 0 ? `${independentSpan} 步` : "熟悉中", note: "不用再看一次，也能照順序點回來的最長路線。" },
      { key: "assisted", icon: "👀", title: "提示後完成", value: assistedSpan > 0 ? `${assistedSpan} 步` : "暫無資料", note: "再看一次或提醒後，可以完成的最長路線。" },
      { key: "replay", icon: "↻", title: "再看一次", value: `${totalReplayCount} 次`, note: "觀察訓練過程使用提示或重看的次數是否逐漸減少。" },
    ],
  };
}
...
function getOverviewTitle({ stars, weightedAccuracyPercent, totalTrials, isTestMode }) {
  if (!totalTrials) return "還在熟悉";
  if (isTestMode) {
    if (stars >= 3) return "這次很穩定";
    if (stars === 2 && weightedAccuracyPercent >= 60) return "已經掌握一些路線";
    return "適合從短路線開始";
  }
  if (stars >= 3) return "3 星穩定通過";
  if (stars === 2) return "2 星正在成形";
  return "1 星需要支持";
}

function buildQuickInterpretation({
  isTestMode,
  independentSpan,
  assistedSpan,
  weightedAccuracyPercent,
  ...
  if (isTestMode) {
    const spanText = independentSpan >= 4
      ? `孩子已經能自己記住 ${independentSpan} 步左右的石頭路。`
      : independentSpan >= 2
      ? `孩子已經能自己完成短路線，約 ${independentSpan} 步。`
      : "孩子目前還在熟悉「看亮燈、記順序、點回來」的規則。";
    const accuracyText = weightedAccuracyPercent >= 80
      ? "整體來看，多數路線都能完成。"
      : weightedAccuracyPercent >= 60
      ? "整體來看，已經能完成一部分路線，但路線變長時還會不穩。"
      : "整體來看，目前適合降低步數，先讓孩子有成功經驗。";
    const sequenceText = sequenceMatchPercent < 60 ? "順序還在練習中，可以先不要增加太多石頭。" : "";
    return `${spanText}${accuracyText}${sequenceText}正式測驗沒有再看一次或提示，所以結果代表孩子在無提示情境下的本次表現。本次結束原因：${stopReasonText}`;
  }

  const starText = stars >= 3
    ? "孩子在目前層級能較穩定地記住路線，錯誤與提示需求都不高。"
    : stars === 2
    ? "孩子已經抓到任務規則，也能完成部分路線，但表現可能受提示、路線長度或注意力波動影響。"
    : "目前層級的負荷較高，孩子可能仍在熟悉看路線、記位置與照順序點回來的流程。";
  const spanText = assistedSpan > independentSpan
    ? `目前約可獨立完成 ${independentSpan || 0} 步路線，在提示或重看後可挑戰到 ${assistedSpan} 步。`
    : `目前約可獨立完成 ${independentSpan || assistedSpan || 0} 步路線。`;
  return `這次屬於「${level.title}」的訓練結果。${starText}主要型態是「${pattern.title}」：${pattern.description}${spanText}`;
}

function getTrainingLevel({ maxAttemptSpan, independentSpan, difficulty }) {
  const raw = String(difficulty || "").toLowerCase();
  const span = Math.max(maxAttemptSpan, independentSpan);
  if (raw.includes("hard") || span >= 5) {
    return { title: "進階空間順序層級", description: "同時處理較長路線、空間位置與點選順序，更考驗記憶與注意維持。" };
  }
  if (raw.includes("normal") || span >= 3) {
    return { title: "穩定路線記憶層級", description: "開始增加路線長度，重點是把多個位置串成一條順序。" };
  }
  return { title: "基礎路線記憶層級", description: "先把少量位置看清楚，再依順序點回來，建立位置與順序的基本連結。" };
}

function getPerformancePattern({
  history,
  weightedAccuracyPercent,
  independentSpan,
  assistedSpan,
  ...
  const errors = getDetailedErrorCounts(history);
  const errorTotal = Math.max(errors.position + errors.sequence + errors.extra, 1);
  if (assistedSpan > independentSpan || replayRate >= 0.35) {
    return { key: "support", title: "提示或重看後能挑戰更長路線", description: "孩子並非完全不會，而是獨立整理整條路線時較容易掉步驟；重看或提醒能把能力拉出來。" };
  }
  if (timeoutCount >= 2 || (totalTrials > 0 && timeoutCount / totalTrials >= 0.3)) {
    return { key: "slow", title: "知道要做，但回想時間比較吃緊", description: "孩子可能有記到路線，但開始作答或回想時需要比較久。" };
  }
  if (errors.position / errorTotal >= 0.35) return { key: "position", title: "空間位置容易記混", description: "這次較容易點到不同位置，空間定位是較需要支持的部分。" };
  if (errors.sequence / errorTotal >= 0.35) return { key: "sequence", title: "記得位置，但順序容易混淆", description: "可能看到了正確石頭，但點選的先後順序還不穩。" };
  if (lateErrorCount > earlyErrorCount && lateErrorCount > 0) return { key: "late", title: "前面記得住，後段步驟容易漏掉", description: "路線後半段較容易忘記或順序斷掉，可能接近本次記憶負荷上限。" };
  if (errors.extra > 0) return { key: "fast", title: "作答時容易多點或太快", description: "孩子可能還沒整理好整條路線就開始點選，需要練習先停一下再出手。" };
  if (weightedAccuracyPercent >= 80 && independentSpan >= 3) return { key: "stable", title: "路線記憶與順序控制都相對穩定", description: "孩子能把看到的路線轉成正確點選，目前層級已接近熟練。" };
  return { key: "forming", title: "路線記憶正在建立中", description: "已經有一些成功表現，位置、順序與作答節奏仍在整合。" };
}

function getDetailedErrorCounts(history) {
  return history.reduce((counts, item) => {
    if (item.correct || item.isCorrect || item.timeout || item.isTimeout) return counts;
    ...
    const type = String(item.errorType || "").toLowerCase();
    if (type.includes("position") || user.some((value) => !target.includes(value))) counts.position += 1;
    else if (type.includes("repeat") || type.includes("random") || user.length > target.length) counts.extra += 1;
    else counts.sequence += 1;
    return counts;
  }, { position: 0, sequence: 0, extra: 0 });
}

function buildInsightCards({ isTestMode, pattern, independentSpan, assistedSpan, weightedAccuracyPercent }) {
  if (isTestMode) {
    return [
      { title: "視覺空間順序記憶", text: `本次最高通過 ${independentSpan || 0} 步，用來觀察能否記住石頭位置並照相同順序點回來。` },
      { title: "作答錯誤與穩定度", text: `${pattern.title}。${pattern.description}` },
      { title: "路線完成狀況", text: `本次路線正確率為 ${weightedAccuracyPercent}%。這是單次遊戲觀察，建議和之後幾次結果一起看。` },
    ];
  }
  return [
    { title: "這次難度", text: "代表這次石頭路的長度、位置量與記憶負荷。" },
    { title: "自己記住幾步？", text: `不重看、不提示時，本次最長可依序完成 ${independentSpan || 0} 步。` },
    { title: "提示後完成幾步？", text: `重看或提醒後，本次最長可完成 ${assistedSpan || 0} 步，可用來觀察提示是否有幫助。` },
    { title: "主要觀察", text: `${pattern.title}。${pattern.description}` },
  ];
}

function getTrainingStarDescription(stars) {
  if (stars >= 3) return "目前層級的路線記憶較穩定，錯誤與提示需求都不高。";
  if (stars === 2) return "已抓到任務規則並能完成部分路線，建議在同層級多累積成功經驗。";
  return "目前層級負荷較高，可先降低步數或增加示範，再逐步減少提示。";
}

function getPatternSuggestion(patternKey) {
  const suggestions = {
    support: "先允許重看一次，成功後改成只用口頭提醒，最後才完全不提示。",
    slow: "先不催速度，改用固定流程：看完整條路、心裡想一次、再開始點。",
    position: "作答前請孩子先用手指空比路線，確認每顆石頭的位置關係。",
    sequence: "練習時加入第一、第二、第三的口語順序，幫助孩子把位置串成路線。",
    late: "維持同一步數，請孩子點完後回想最後一顆在哪裡，補強後段保持。",
    fast: "加入「看完停一拍再點」的規則，先把多點或急點降下來。",
    stable: "可嘗試進入下一個步數，但一次只增加一個難度。",
    forming: "先固定同一個步數與節奏，讓孩子形成自己的記憶策略。",
  };
  return suggestions[patternKey] || "先穩定完成目前步數，再慢慢增加一步或減少提示。";
}

function getStopReasonText(reason) {
  const value = String(reason || "").toLowerCase();
  if (value.includes("timeout")) return "連續 2 題沒有在時間內完成，因此測驗結束。";
  if (value.includes("wrong") || value.includes("failure")) return "同一記憶數量連續 2 題沒有完成，因此測驗結束。";
  if (value.includes("max_memory") || value.includes("max_length")) return "孩子已完成本次測驗的最高記憶數量。";
  return "測驗已完成。";
}

function getStopReasonLabel(reason) {
  const value = String(reason || "").toLowerCase();
  if (value.includes("timeout")) return "連續逾時";
  if (value.includes("wrong") || value.includes("failure")) return "同長度未完成";
  if (value.includes("max_memory") || value.includes("max_length")) return "完成最高長度";
  return "測驗完成";
}

function getSystemSummary(state, scoring) {
  const value = state?.aiSummary || scoring?.aiSummary || state?.parentSummary || scoring?.parentSummary;
  if (typeof value === "string") return value;
  if (value && typeof value === "object") return value.reason || value.summary || value.text || "";
  return "";
}
...
const styles = {
  ...
  insightCard: {
    ...
    boxShadow: "0 8px 18px rgba(70, 45, 25, 0.06)",
    textAlign: "left",
  },
  ...
  ruleCard: {
    ...
    border: "2px solid rgba(85, 132, 172, 0.18)",
    textAlign: "left",
  },
  ...
  disclaimer: {
    ...
    border: "2px dashed rgba(92, 64, 51, 0.2)",
    color: "#5c4033",
    textAlign: "left",
  },
