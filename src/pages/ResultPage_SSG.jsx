...
const TRAINING_ROUTE = "/training-ssg";
const HAT_GAME_ROUTE = "/hat-sticker-game";

/* ========= 帽子遊戲排程 =========
 * 只有 training 結果會進入排程。
 * TestPage_SSG / test 結果不會累計，也不會觸發帽子遊戲。
 */
...
function getSsgResultSessionId(result) {
  return String(
    result?.sessionId ||
    result?.resultId ||
    result?.finishedAt ||
...
/**
 * 登記一次 SSG 訓練完成紀錄，並回傳這次是否取得帽子遊戲資格。
 * 同一份結果即使重新整理，也不會重複累計。
 */
function registerSsgHatReward(result) {
  if (!result || result.mode !== "training") return null;

  const childId = resolveChildId(result);
  const resultSessionId = getSsgResultSessionId(result);
  if (!resultSessionId) return null;

  const scheduleKey = `hatRewardSchedule_${childId}`;
  const processedKey = `hatRewardProcessed_SSG_${childId}`;
  ...
  if (processedIds.includes(resultSessionId)) {
    const pending = safeJsonParse(
      sessionStorage.getItem(`pendingHatReward_SSG_${childId}`),
      null
    );
  ...
  const rewardSessionId = `SSG-${childId}-${resultSessionId}`;
  const reward = {
    childId,
    gameId: "SSG",
    sourceMode: "training",
  ...
  sessionStorage.setItem(`pendingHatReward_SSG_${childId}`, JSON.stringify(reward));
  return reward;
}
...
function readSavedResult() {
  try {
    return getStoredSsgResult();
  } catch (error) {
    console.error("讀取 SSG 結果失敗：", error);
    return null;
  }
}
...
function getScoring(rawResult) {
  ...
  return calculateSsgScore(rawResult.records, {
    mode: rawResult.mode,
...
function normalizeResult(rawResult) {
  ...
  const aiAnalysis = source.aiAnalysis || source.ssgAnalysis || {};
...
function getDetailedSsgStars(result = {}, displayScore = 0) {
  const stars = result?.scoring?.stars ?? result?.stars;
...
function getDetailedSsgLevelInfo(result = {}) {
...
function getDetailedSsgStarInfo(stars) {
  if (stars >= 3) {
    return {
      title: "目前層級表現穩定",
      meaning:
...
function getDetailedSsgProfile(result = {}) {
...
function buildDetailedSsgTrainingOneSentence(result = {}, stars = 1) {
...
function buildDetailedSsgTrainingSummary(result = {}, stars = 1) {
...
function buildDetailedSsgTrainingNextSuggestion(result = {}) {
  const profile = getDetailedSsgProfile(result);
  const levelInfo = getDetailedSsgLevelInfo(result);
  const nextLabel =
...
/* ========= 主頁面 ========= */
export default function ResultPage_SSG() {
  ...
  const result = useMemo(() => {
    try {
      ...
      console.error("Failed to parse SSG result:", error);
      return null;
    }
  }, [location.state]);

  useEffect(() => {
    // 明確限制：只有 SSG 訓練結果可登記帽子遊戲。
    // 測驗結果（mode === "test"）直接略過。
    ...
    try {
      setHatReward(registerSsgHatReward(result));
    } catch (error) {
      console.error("SSG 帽子遊戲排程登記失敗：", error);
      setHatReward(null);
    }
  }, [result]);
  ...
  const handleBackToMenu = () => {
    ...
    if (result?.mode === "training" && hatReward) {
      sessionStorage.removeItem(`pendingHatReward_SSG_${hatReward.childId}`);
  ...
  if (!result) {
    return (
      <div className="ssg-result-page" style={{ backgroundImage: `linear-gradient(rgba(255,255,255,0.22), rgba(255,255,255,0.22)), url(${backgroundImg})` }}>
        ...
        <main className="ssg-result-main-card ssg-result-empty-card">
          <header className="ssg-result-header">
            <p className="ssg-mode-tag">結果頁</p>
            <h1 className="ssg-result-main-title">找不到結果</h1>
          </header>
          <section className="ssg-note-box">
            <h3>目前沒有 SSG 資料</h3>
            <p>請先完成一次測驗或訓練，這裡就會出現家長可以閱讀的結果整理。</p>
          </section>
          <footer className="ssg-action-btns">
            <button type="button" className="ssg-image-button" disabled={isNavigating} onClick={() => handleNavigate(TEST_MAP_ROUTE)} aria-label="回到主頁">
              <img width={1024} height={341} src={homeBackBtn} alt="回到主頁" />
            </button>
          </footer>
        </main>
      </div>
  ...
  const detailedTrainingStars = getDetailedSsgStars(result, displayScore);
  ...
  const parentSummaryText = isTraining
    ? buildDetailedSsgTrainingSummary(result, detailedTrainingStars)
  ...
  return (
    <div className="ssg-result-page" style={{ backgroundImage: `linear-gradient(rgba(255,255,255,0.22), rgba(255,255,255,0.22)), url(${backgroundImg})` }}>
      ...
      <main className="ssg-result-main-card">
        <header className="ssg-result-header">
        ...
        <section className="ssg-overview-card">
          <div className="ssg-overview-left">
            <div className="ssg-score-circle" aria-label={`${displayScore} 分`}>
              <span className="ssg-score-number">{displayScore}</span>
              <span className="ssg-score-unit">分</span>
            </div>
            <div className="ssg-overview-text-box">
              <p className="ssg-overview-label">本次表現</p>
              <h2 className="ssg-overview-title">{childShortLabel}</h2>
              <p className="ssg-overview-desc">{oneSentenceResult}</p>
            </div>
          </div>

          <div className="ssg-star-summary">
            <div className="ssg-star-row" aria-label={`${stars} 顆星`}>
              {[1, 2, 3].map((star) => (
                <span key={star} className={`ssg-star-chip ${star <= stars ? "is-on" : ""}`}>★</span>
              ))}
            </div>
            <p>星星代表本次完成表現</p>
        ...
        <section className="ssg-ai-card">
          ...
          {result.suggestedAction && <p className="ssg-ai-extra">{result.suggestedAction}</p>}
        </section>
        ...
        <footer className="ssg-action-btns">
          <button type="button" className="ssg-image-button" disabled={isNavigating} onClick={handleBackToMenu} aria-label="回到主頁">
            <img width={1024} height={341} loading="lazy" src={homeBackBtn} alt="回到主頁" />
          </button>
          {isTraining && (
            <button type="button" className="ssg-image-button" disabled={isNavigating} onClick={() => handleNavigate(TRAINING_ROUTE)} aria-label="play again">
...
.ssg-result-empty-card { max-width: 760px; }
.ssg-result-header { text-align: center; margin-bottom: 24px; }
.ssg-mode-tag {
  display: inline-flex;
  align-items: center;
...
.ssg-overview-left { display: flex; align-items: center; gap: 24px; text-align: left; min-width: 0; }
.ssg-score-circle {
...
.ssg-score-number { font-size: clamp(40px, 6vw, 62px); font-weight: 900; line-height: .95; color: #71471d; }
.ssg-score-unit { margin-top: 4px; font-size: 16px; font-weight: 800; color: #8b653b; }
.ssg-overview-label, .ssg-card-label, .ssg-stat-label, .ssg-ai-label { margin: 0 0 6px; color: #9b744b; font-size: 14px; font-weight: 800; }
.ssg-overview-title { margin: 0 0 8px; font-size: clamp(25px, 3vw, 34px); color: #5d4028; }
.ssg-overview-desc { margin: 0; font-size: clamp(16px, 2vw, 20px); line-height: 1.65; color: #6b523d; }
.ssg-star-summary { min-width: 230px; text-align: center; padding-left: 24px; border-left: 1px solid #ead8bd; }
.ssg-star-row { display: flex; justify-content: center; gap: 8px; font-size: clamp(31px, 4vw, 43px); }
.ssg-star-chip { color: #ded4c5; filter: drop-shadow(0 2px 0 rgba(121, 75, 18, .08)); }
.ssg-star-chip.is-on { color: #f0b538; }
.ssg-star-summary p { margin: 8px 0 4px; font-weight: 800; color: #6a4a2b; }
.ssg-star-summary small { color: #8c7359; line-height: 1.45; }
...
.ssg-quick-stats { grid-template-columns: repeat(3, minmax(0, 1fr)); margin: 18px 0; }
.ssg-highlight-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
.ssg-indicator-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
.ssg-ai-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }

.ssg-stat-card,
.ssg-observation-card,
.ssg-panel-block,
.ssg-ai-card,
.ssg-note-box { border-radius: 18px; padding: clamp(17px, 2vw, 22px); }
.ssg-stat-card:nth-child(3n + 1) { border-top: 4px solid #a7c85a; }
.ssg-stat-card:nth-child(3n + 2) { border-top: 4px solid #78bfe8; }
.ssg-stat-card:nth-child(3n) { border-top: 4px solid #ed9c82; }
.ssg-stat-value { margin: 0 0 8px; font-size: clamp(24px, 3vw, 32px); font-weight: 900; color: #4d3827; }
...
.ssg-status-pill.good { background: #e8f4cf; color: #4f742d; border-color: #c9dda0; }
.ssg-status-pill.watch { background: #fff0bd; color: #80601d; border-color: #ead497; }
.ssg-status-pill.alert { background: #fde1d8; color: #93483b; border-color: #edc2b7; }
.ssg-status-pill.neutral { background: #dff1fb; color: #426f8e; border-color: #bfdeef; }
.ssg-indicator-score { font-size: 23px; font-weight: 900; color: #4b3828 !important; margin-bottom: 8px !important; }
.ssg-card-meaning { margin-top: 9px !important; color: #6d6a2e !important; font-weight: 700; }
.ssg-score-bar { width: 100%; height: 10px; border-radius: 999px; background: #eee5d9; overflow: hidden; margin: 8px 0 12px; }
.ssg-score-bar span { display: block; height: 100%; border-radius: inherit; background: linear-gradient(90deg, #9dc85a, #e7b348); }
.ssg-ai-card { background: linear-gradient(135deg, rgba(237, 247, 212, .88), rgba(224, 242, 251, .9)); border-color: rgba(128, 169, 119, .38); }
.ssg-ai-extra { margin-top: 13px !important; padding-top: 13px; border-top: 1px dashed rgba(102, 122, 75, .35); }
.ssg-note-box { background: rgba(255, 252, 239, .95); }
.ssg-note-box--warning { background: rgba(255, 244, 210, .96); border-color: rgba(220, 174, 74, .45); }
.ssg-note-box h3 { margin: 0 0 8px; font-size: 21px; color: #594129; }
...
.ssg-image-button:hover:not(:disabled) { transform: translateY(-3px); filter: brightness(1.05); }
.ssg-image-button:focus-visible { outline: 3px solid rgba(106, 168, 207, .55); outline-offset: 5px; border-radius: 14px; }
.ssg-image-button:disabled { opacity: .55; cursor: not-allowed; }
.ssg-image-button img { width: clamp(118px, 15vw, 176px); height: auto; display: block; }
...
@media (max-width: 680px) {
  .ssg-result-page { padding: 10px; background-attachment: scroll; }
  .ssg-result-main-card { width: 100%; padding: 18px 14px 22px; border-width: 2px; border-radius: 20px; }
  .ssg-overview-card { flex-direction: column; gap: 18px; padding: 18px; }
  .ssg-overview-left { align-items: flex-start; }
  .ssg-score-circle { width: 96px; height: 96px; border-width: 5px; }
  .ssg-star-summary { min-width: 0; padding: 16px 0 0; border-left: 0; border-top: 1px solid #ead8bd; }
  .ssg-quick-stats,
  .ssg-highlight-grid,
  .ssg-indicator-grid,
  ...
  .ssg-image-button img { width: min(40vw, 150px); }
}

@media (max-width: 390px) {
  .ssg-overview-left { flex-direction: column; }
  .ssg-action-btns { gap: 8px; }
}
`;
